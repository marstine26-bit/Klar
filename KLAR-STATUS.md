...
|---|---|---|
| **Payment processor — Dodo integration built, needs 2 things from you** | ✅ Dodo Payments approved (09-24). Full backend + client rewrite shipped, security-reviewed (webhook replay protection, reverse-tabnabbing fix, cancellation-error surfacing). **All 8 real products now created and wired up** (09-25): Essential/Pro × Monthly/Annual × GBP/ZAR — `pdt_0NoJDtd4UIDCgzYaptMvc` (Essential Monthly GBP £3.99), `pdt_0NoJEEMSLOCo6C8sqztxY` (Essential Annual GBP £34.99), `pdt_0NoJEVP7EHKn8c4mCircV` (Pro Monthly GBP £7.99), `pdt_0NoJFEKVrHFJqOAdEPyFJ` (Pro Annual GBP £64.99), `pdt_0NoJFRhpBtlzppOv1pfRz` (Essential Monthly ZAR R49), `pdt_0NoJFirrvgRlUTdI9dEFf` (Essential Annual ZAR R499), `pdt_0NoKAMoDsgJb9uY3nWYy8` (Pro Monthly ZAR R89), `pdt_0NoKAdYYZ3pUHv3TZRwjI` (Pro Annual ZAR R899) — all mapped into `DODO_PRODUCTS`/`DODO_PRODUCT_TO_PLAN`, both edge functions redeployed. Family plan intentionally not sold (comingSoon). **Startbutton still pending**, a 3rd application was also in progress as of the last update. | **2 things left**: (1) register `dodo-webhook`'s URL as a webhook endpoint in Dodo's dashboard, generate the signing secret, add it as `DODO_WEBHOOK_SECRET` in Supabase → Edge Functions → Secrets; (2) add a `DODO_API_KEY` secret (used by both checkout creation and delete-account cancellation) — same Dashboard → Secrets page. Fire one real test event from Dodo's dashboard afterward and check the `dodo-webhook` function logs — the webhook payload's exact field names were not confirmed against a live event when this was built, so double-check they match (defensive fallbacks are in place, but verify). |
...
**Running bug tally this review cycle (2026-09-05 → 09-25): 27 real bugs found and fixed** across 24 specialist passes.
